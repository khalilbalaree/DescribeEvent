...
#
# Run all experiment variants for amazon_review_events (all categories together)
...
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR"
echo ""; sleep 10

echo "=== Experiment 2: Without type_text ==="
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --no-type-text
echo ""; sleep 10

echo "=== Experiment 3: Anonymous types (pure, no text) ==="
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --anon-types
echo ""; sleep 10

echo "=== Experiment 4: Anonymous types + type_text ==="
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --anon-with-text
echo ""
