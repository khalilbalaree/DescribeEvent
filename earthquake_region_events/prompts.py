...
Valid event types: Atlantic-Africa, Central-South-America, Central-South-Asia, Mediterranean-Middle-East, NW-Pacific, North-America, SE-Asia-Oceania, SW-Pacific.

Each event shows: [type] +Xd (days since previous event). Use both the event types and the +Xd time gaps to predict what happens next and when.

Response format (follow exactly):
Brief reasoning here (2-3 sentences max).
{"event_type": "<type>", "time_days": <positive_number>}"""
...
Each event shows: [type] +Xd (days since previous event) | description, where description provides details about the event (may be truncated). Use the event types, their descriptions, and the +Xd time gaps to predict what happens next and when.
...
from config import TYPE_TO_ID as _TYPE_TO_ID
_anon_list = ", ".join(_TYPE_TO_ID.values())
