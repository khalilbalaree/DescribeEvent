...
#
# Run all experiment variants for earthquake_region_events
...
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --engine vllm --no-type-text
echo ""; sleep 10

echo "=== Experiment 3: Anonymous types (pure, no text) ==="
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --engine vllm --anon-types
echo ""; sleep 10

echo "=== Experiment 4: Anonymous types + type_text ==="
$CONDA_RUN python "$PARENT_DIR/inference.py" --config-dir "$SCRIPT_DIR" --results-dir "$RESULTS_DIR" --engine vllm --anon-with-text
echo ""
