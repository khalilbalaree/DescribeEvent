...
#
from config import EVENT_TYPES, TIME_SUFFIX, TIME_KEY
...
Each event shows: [type] +X{TIME_SUFFIX} ({TIME_KEY.replace('_', ' ')} since previous event). Use both the event types and the +X{TIME_SUFFIX} time gaps to predict what happens next and when.

Response format (follow exactly):
Brief reasoning here (2-3 sentences max).
{{"event_type": "<type>", "{TIME_KEY}": <positive_number>}}"""
...
Each event shows: [type] +X{TIME_SUFFIX} ({TIME_KEY.replace('_', ' ')} since previous event) | description, where description provides details about the event (may be truncated). Use the event types, their descriptions, and the +X{TIME_SUFFIX} time gaps to predict what happens next and when.
